...
import {XModelType} from './XModelType'

export type XModelPartBoneTransform = {
    position: Vec3
    rotation: Quat
}

export type XModelPartBone = {
    name: string
    parent: number
...
export class XModelPartLoader {
    ...
    private updateBoneNamesAndTransforms(xmodelPart: XModelPart, boneCount: number, rootBoneCount: number, viewhandTable: (boneName: string) => Vec3 | null): void {
        for (let i = 0; i < rootBoneCount + boneCount; i++) {
            ...
            const currentBone = {...xmodelPart.bones[i], name: boneName} as XModelPartBone
            ...
            if (currentBone.parent > -1) {
                const parentBone = xmodelPart.bones[currentBone.parent] as XModelPartBone
