...
import {XModelPart, XModelPartBone} from './XModelPart'

export type XModelSurfWeight = {
    bone: number
    influence: number
}

export type XModelSurfVertex = {
    normal: Vec3
    color: Color
    uv: Vec2
    bone: number
...
export type XModelSurfSurface = {
    vertices: XModelSurfVertex[]
    triangles: number[]
}

export type XModelSurf = {
    name: string
    ...
    private async loadV20(xmodelSurf: XModelSurf, xmodelPart?: XModelPart): Promise<void> {
        ...
        for (let s = 0; s < surfaceCount; s++) {
            ...
            for (let i = 0; i < vertexCount; i++) {
                ...
                if (xmodelPart) {
                    const xmodelPartBone = xmodelPart.bones[vertexBoneIdx] as XModelPartBone
                    position = vec3_add(
                        vec3_rotate(position, xmodelPartBone.worldTransform.rotation),
                        xmodelPartBone.worldTransform.position,
                    )
